...
```
[provideros] The resolved Codex CLI is older than the client that wrote the account model cache
```
...
STATE_DIR="${MODEL_ROUTER_STATE_DIR:-${CODEX_ROUTER_STATE_DIR:-${KIMI_CODEX_STATE_DIR:-${HOME}/.codex/provideros}}}"
...
`$CODEX_HOME/provideros/merged-models.json`.
...
```sh
grep -c estimatedInputTokens "$CODEX_HOME/provideros/usage-events.jsonl"
```
...
```sh
grep -A2 '"contextWindow"' "$CODEX_HOME/provideros/user-models.json"
```
...
```sh
grep -c emptyCompletionRetried "$CODEX_HOME/provideros/usage-events.jsonl"
```
...
```sh
launchctl print "gui/$(id -u)/io.github.provideros"
./bin/doctor --fix
```
...
```sh
systemctl --user status provideros.service
journalctl --user -u provideros.service --since today
./bin/doctor --fix
```
...
The task runs `start-provideros-hidden.vbs` from the state directory under
`wscript.exe`, which starts `start-provideros.cmd` without a console window,
