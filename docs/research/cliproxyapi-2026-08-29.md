...
publish provideros's selected models into Cursor Agent and Cursor App?
...
putting CLIProxyAPI between Cursor Agent and provideros would not make the CLI
work. It would solve only the OpenAI-compatible half of Cursor App, which is
small enough to translate directly before re-entering provideros's canonical
Responses path.

## Security and operational fit

CLIProxyAPI can bind beyond loopback when its `host` is left empty. That is a
reasonable server default but conflicts with provideros's local caller-
...
It would bypass or duplicate provideros behavior such as provider selection,
