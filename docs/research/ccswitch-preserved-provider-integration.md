...
from `custom` to `provideros-signed`, which changes Codex Desktop's task
...
- The Codex state database currently contains 2,475 tasks tagged `custom`, 94
  tagged `openai`, 7 tagged `provideros-signed`, 1 tagged `provideros`, and 1
...
- CC Switch currently regards `codex-official` as the selected Codex profile.
  ...
  was active and now contains stale `provideros-signed`/Router-managed fields.
...
model = "gpt-5.6-sol"
model_catalog_json = "/Users/rohitsabu/.codex/provideros/merged-models.json"
...
from the rejected `provideros-signed` attempt. An isolated copy of the probe
