...
storage mode, where both sides use only the `group.io.github.provideros` App
...
```bash
# Set icon-only mode
defaults write io.github.provideros.tray ModelRouterTray.menuBarDisplayMode iconOnly

# Toggle model name visibility
defaults write io.github.provideros.tray ModelRouterTray.menuBarShowModelName -bool false

# Set icon style (router, provider, indicator, preset, custom)
defaults write io.github.provideros.tray ModelRouterTray.menuBarIconStyle router
defaults write io.github.provideros.tray ModelRouterTray.menuBarPresetIcon sparkles
```
