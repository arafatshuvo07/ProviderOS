...
3. **A completed local verification.** All five checks passing in one run on
   this machine, recorded in `~/.codex/provideros/multi-agent-proofs.json`.
...
```bash
bin/provideros codex subagents set <provider>/<model> on
...
```bash
bin/provideros codex subagents certify <provider>/<model> [<provider>/<model> ...]
```
