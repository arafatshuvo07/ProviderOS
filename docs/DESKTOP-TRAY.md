...
`%LOCALAPPDATA%\provideros` is allowed), then performs a UAC-elevated DACL
...
location (`%LOCALAPPDATA%\provideros` on Windows and
`~/.local/share/provideros` on Linux). It displays a useful offline state when
