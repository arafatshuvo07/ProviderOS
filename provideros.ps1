...
function Get-ObsoleteTauriExecutableForRemoval {
  ...
  $LegacyBinary = Join-Path $Cursor "provideros-desktop.exe"
...
function Test-RecognizedControlCenterTaskAction([string]$Execute, [string]$Argument) {
  ...
  $TauriAction = $ExpandedExecute.EndsWith(
    "apps\desktop\src-tauri\target\release\provideros-desktop.exe",
...
switch ($Command) {
  ...
  "start" {
    if ($Arguments.Count -eq 0) {
      ...
      throw "Usage: provideros.ps1 start [--foreground]."
    }
  }
