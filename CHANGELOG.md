...
- **Antigravity OAuth now uses an operator-owned, fail-closed sign-in.** The
  ...
  with the truthful `provideros` identity; incompatible legacy records are
...
- **OpenClaw is now a one-click routed client.** The Harness page can install
  ...
  selected router model through a private `provideros` Responses provider.
...
- **Homebrew installs can reach every command again.** The formula's PATH shim
  ...
  custom provider's models, and a bare `provideros` or `provideros --help`
  printed `model-router`'s usage instead of the packaged command list. The shim
  now dispatches through `bin/provideros`, which exists for exactly this
  case. `post_install` gets its own private entry point, because
  `bin/provideros` refuses `install` by design. Reported in #334.
...
- **The routed-model skill says a text-only turn ends the task.** Custom
  ...
  Codex then hands control back. The `provideros` skill now states that
...
- **A command that opens the browser panel.** The panel shipped with no way to
  ...
  "nothing to install" still meant "and no way in". `provideros.ps1 panel`
...
- **The companion no longer requires a Rust toolchain.** Building it meant
  ...
  required, and `provideros.ps1 companion` selects it explicitly.
...
- **Every single-argument Windows subcommand was unreachable.** PowerShell
  ...
  character. `provideros.ps1 tray status` died on "Unknown tray action 's'",
...
- **The desktop companion is a download now, not a build.** It could be
  ...
  `provideros-tray-<version>-windows-x64.exe` and the Linux binary,
...
- **The Windows tray is managed the way the macOS one is.** Installing it was
  ...
  "use scripts/build-desktop-tray.ps1" and `provideros.ps1` had no `tray`
  ...
  way to check, restart, or remove it. `./provideros.ps1 tray
...
- **`update check` no longer performs the update.** The `bin/update` wrapper
  ...
  the router instead. Both `bin/update` and `provideros.ps1 update` now
