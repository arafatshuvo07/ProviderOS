// Install the provideros skill pack into the Codex app's user-skill
...
// both the marker and a protected ownership file under provideros's private
...
const MARKER = ".provideros-managed";
...
const RETIRE_PREFIX = ".provideros-retire-";
const RETIRE_JOURNAL = ".provideros-retirement.json";
...
const PUBLISH_MARKER = ".provideros-publishing";
...
export function skillOwnershipPath(codexHome) {
  return path.resolve(codexHome) === path.resolve(CODEX_HOME)
    ...
    : path.join(codexHome, "provideros", "managed-skills.json");
}
...
function preservedSkillPath(target) {
  for (;;) {
    const candidate = `${target}.provideros-preserved-${randomBytes(8).toString("hex")}`;
...
function stageManagedSkill(source, target, name, token, provenance, { onStaged } = {}) {
  const staging = mkdtempSync(path.join(path.dirname(target), ".provideros-install-"));
...
function approveExternalSkillsUnlocked(codexHome, names, { quiet = false } = {}) {
  ...
  for (const name of requested) {
    ...
    if (ownershipEvidence(codexHome, name, ownership).owned) {
      throw new Error(`Cannot approve "${name}" as external because provideros owns it.`);
    }
    const target = path.join(codexSkillsDir(codexHome), name);
  ...
  for (const { name, targetDigest, sourceDigest } of approvals) {
    // Approval explicitly transfers responsibility away from provideros.
  ...
  if (!quiet) {
    console.error(
      `provideros: approved ${approvals.length} external skill(s); exact content changes require re-approval.`,
    );
  }
...
function revokeExternalSkillsUnlocked(codexHome, names, { quiet = false } = {}) {
  ...
  if (!quiet && removed.length > 0) {
    console.error(`provideros: revoked ${removed.length} external skill approval(s).`);
  }
  return removed;
}
...
export function skillRequiredFields() {
  ...
  try {
    ...
    const match = /<!--\s*provideros-required-fields:\s*(\{[\s\S]*?\})\s*-->/.exec(text);
...
function installSkillsUnlocked(
  ...
  if (!existsSync(sourceRoot)) {
    if (!quiet) {
      console.error("provideros: no skills/ directory in this checkout; nothing to install.");
    }
  ...
  for (const entry of readdirSync(sourceRoot, { withFileTypes: true })) {
    ...
    if (!existsSync(path.join(source, "SKILL.md"))) {
      if (!quiet) console.error(`provideros: skipping ${entry.name} (no SKILL.md).`);
      continue;
    }
    ...
    if (destStat && !evidence.owned && coexistence.approved) {
      ...
      if (!quiet) {
        console.error(
          `provideros: using approved external skill "${entry.name}"; content preserved.`,
        );
      }
      continue;
    }
    if (destStat && !evidence.owned) {
      ...
      if (!quiet) {
        console.error(
          `provideros: not overwriting existing skill "${entry.name}" (${evidence.reason}); existing content preserved.`,
        );
      }
      skipped += 1;
      continue;
    ...
    try {
      if (destStat) {
        ...
        if (!retired.retired) {
          ...
          if (!quiet) {
            console.error(
              `provideros: not replacing skill "${entry.name}" (${retired.reason}); existing content preserved.`,
            );
          }
          continue;
        }
      ...
      if (!publication.published) {
        ...
        if (!quiet) {
          console.error(
            `provideros: not replacing skill "${entry.name}" (${publication.reason}); existing content preserved.`,
          );
        }
        continue;
      }
  ...
  for (const name of Object.keys(ownership.skills)) {
    ...
    if (!retired.retired) {
      ...
      if (!quiet) {
        console.error(
          `provideros: stale ownership for "${name}" was cleared (${retired.reason}); existing content preserved.`,
        );
      }
      continue;
    }
    ...
    if (!quiet) {
      console.error(
        removed
          ? `provideros: removed stale managed skill "${name}".`
          : `provideros: stale managed skill "${name}" changed during removal and was preserved.`,
      );
    }
  }
  for (const name of Object.keys(ownership.external)) {
    ...
    if (!quiet) {
      console.error(
        `provideros: revoked obsolete external approval for "${name}"; existing content preserved.`,
      );
    }
  }
...
function uninstallSkillsUnlocked(
  ...
  for (const name of Object.keys(ownership.skills)) {
    ...
    if (!retired.retired) {
      ...
      if (!quiet) {
        console.error(
          `provideros: not removing skill "${name}" (${retired.reason}); existing content preserved.`,
        );
      }
      continue;
    }
    ...
    if (commitRetirement(retired.transaction)) {
      ...
      console.error(
        `provideros: not removing skill "${name}" (content changed after quarantine); existing content preserved.`,
      );
    }
  }
  ...
  if (!quiet && removed > 0) {
    console.error(`provideros: removed ${removed} verified managed skill(s).`);
  }
  return removed;
}
  ...
  if (!commands.includes(command)) {
    ...
    try {
      if (command === "install") {
        ...
        console.error(
          `provideros: installed ${installed} skill(s) into ${codexSkillsDir(codexHome())}${
        ...
        console.error("provideros: provideros skills removed.");
      ...
      console.error(`provideros: skill ${command} failed: ${error.message}`);
      process.exitCode = 2;
    }
  }
}
