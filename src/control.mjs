...
async function handleSubagents(action, value, flag, rest = []) {
  ...
  if (action === "select-all") {
    ...
    if (!existsSync(CALLER_SECRET_PATH)) {
      throw new Error(
        "The local router caller key is missing; run ./bin/doctor --fix (.\\provideros.ps1 doctor --fix on Windows).",
      );
    }
...
function handleTray(action) {
  ...
  if (value === "rebuild") {
    ...
    // `provideros.ps1 tray rebuild`, which owns the same unified Electron
    ...
    const result = process.platform === "win32"
      ? spawnSync(
          ...
          [
            ...
            "Bypass",
            "-File",
            path.join(REPO_ROOT, "provideros.ps1"),
            "tray",
            "rebuild",
            "--preserve-window",
          ],
