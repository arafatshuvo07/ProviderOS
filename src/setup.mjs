...
function oauthSetupHint(provider) {
  ...
  if (provider.id === "antigravity-oauth") {
    const command = process.platform === "win32"
      ? ".\\provideros.ps1 providers login antigravity-oauth"
      : "./bin/providers login antigravity-oauth";
    const probe = process.platform === "win32"
      ? ".\\provideros.ps1 providers probe antigravity-oauth --live --yes"
  ...
  try {
    if (process.platform === "darwin") {
      ...
      run("powershell.exe", [
        ...
        "Bypass",
        "-File",
        path.join(SOURCE_ROOT, "provideros.ps1"),
        "tray",
        "install",
      ]);
    } else {
    ...
    process.stdout.write(
      `Desktop companion install did not finish: ${error instanceof Error ? error.message : String(error)}\n` +
        ...
        (process.platform === "win32"
          ? "The router itself is installed; retry later with .\\provideros.ps1 tray.\n"
        ...
        "The companion also runs in a browser: .\\provideros.ps1 panel (./bin/panel on macOS and Linux).\n",
    );
  }
}
