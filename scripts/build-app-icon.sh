...
if [ "$(uname -s)" != "Darwin" ]; then
  printf 'provideros: the icon set is built with macOS sips/iconutil.\n' >&2
  exit 1
fi
if [ ! -f "$source_svg" ]; then
  printf 'provideros: missing icon source %s\n' "$source_svg" >&2
  exit 1
fi
...
render() {
  ...
  if [ ! -s "$iconset/$name" ]; then
    printf 'provideros: sips could not rasterize %s at %spx.\n' "$source_svg" "$size" >&2
    exit 1
  fi
}
