...
Router state lives under `$CODEX_HOME/provideros` by default:
...
- Creates `~/.codex/config.toml.pre-provideros` before its first change.
...
```sh
diff -u ~/.codex/config.toml.pre-provideros ~/.codex/config.toml
```
