...
function windowsSwitchBranches(source) {
  ...
  assert.notEqual(start, -1, "provideros.ps1 must dispatch on $Command");
...
test(
  ...
  () => {
    ...
    const testRoot = mkdtempSync(path.join(os.tmpdir(), "provideros-desktop-path-"));
...
test("the Windows wrapper hands every command its own arguments", () => {
  ...
  const branches = windowsSwitchBranches(
    readFileSync(path.join(root, "provideros.ps1"), "utf8"),
  );
...
test("Windows exposes signed-routing and the shared refresh transaction", () => {
  const windows = readScript("provideros.ps1");
...
test("prepare-only exits before the skills step", () => {
  const source = readScript("bin", "install");
  const prepareExit = source.indexOf("Dependencies and local ProviderOS files are prepared");
