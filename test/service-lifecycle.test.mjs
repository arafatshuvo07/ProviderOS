...
test("Windows start dispatches managed and foreground modes without bypassing lifecycle ownership", { skip: process.platform !== "win32" }, () => {
  ...
  try {
    execFileSync("powershell.exe", ["-NoLogo", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", path.join(root, "provideros.ps1"), "start"], { env, encoding: "utf8" });
    let lines = readFileSync(log, "utf8").trim().split(/\r?\n/);
    assert.equal(lines.length, 1);
    assert.match(lines[0], /src\\service\.mjs start$/i);

    writeFileSync(log, "");
    execFileSync("powershell.exe", ["-NoLogo", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", path.join(root, "provideros.ps1"), "start", "--foreground"], { env, encoding: "utf8" });
