...
test("the Windows operational scripts parse in Windows PowerShell", { skip: process.platform !== "win32" }, () => {
  for (const name of [
    "install.ps1",
    "deploy-provideros.ps1",
    "restart-provideros.ps1",
    "provideros.ps1",
...
test("the Windows restart helper uses the supported service transaction", () => {
  const source = readScript("restart-provideros.ps1");
...
test("the local deploy helper copies source without purging target-only files", () => {
  const source = readScript("deploy-provideros.ps1");
  ...
  assert.match(source, /\.provideros-deploy-manifest\.json/);
...
test("deploy and install preserve the existing tray opt-in", () => {
  const deploy = readScript("deploy-provideros.ps1");
  ...
  const trayInstall = deploy.indexOf('provideros.ps1") tray install', trayStop);
  const trayVerify = deploy.indexOf("$TrayStatus = Get-TrayStatus", trayInstall);
  const success = deploy.indexOf("ProviderOS published, installed, and verified.");
  ...
  assert.match(deploy, /apps\\control-center\\release\\win-unpacked\\ProviderOS\.exe/);
  ...
  const refresh = install.indexOf("provideros.ps1\") tray install");
  ...
  assert.match(install, /provideros\.ps1 tray repair/);
