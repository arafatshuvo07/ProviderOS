...
test("the dispatcher resolves a chain of symlinks", { skip: posixOnly }, () => {
  // Homebrew stacks two: `bin/provideros` points at `opt/<name>/...`, which is
...
test("the POSIX dispatcher covers the Windows command set", () => {
  // provideros.ps1 is the same entry point for Windows. A command reachable on
  ...
  const windows = readFileSync(path.join(root, "provideros.ps1"), "utf8");
  const declared = windows.match(/\$Commands = @\(([\s\S]*?)\)/);
  assert.ok(declared, "provideros.ps1 must declare $Commands");
  ...
  for (const command of commands) {
    ...
    assert.ok(statSync(target), `provideros.ps1 offers ${command} but its POSIX entry point is missing: ${target}`);
    if (!posixOnly) {
      assert.ok(
        statSync(target).mode & 0o111,
        `provideros.ps1 offers ${command} but its POSIX entry point is not executable: ${target}`,
      );
    }
  }
});
