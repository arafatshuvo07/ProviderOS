...
- Harness: one row each for OpenClaw, Cursor, Claude Code, Gemini CLI, DeepSeek Harness, and Codex. Every row
  ...
  the router-owned `provideros` provider in the same click.
...
it, the install manifest, or the stable user checkout: `%LOCALAPPDATA%\provideros`
on Windows and `${XDG_DATA_HOME:-~/.local/share}/provideros` on macOS/Linux.
