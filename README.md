...
| `bin/` | ProviderOS operator commands and client integrations |
...
`~/.local/share/provideros` and `~/.codex/provideros` locations during
...
ProviderOS deliberately keeps the established `CODEX_ROUTER_*` and related
compatibility environment variables, state path, service labels, IPC protocol,
provider IDs, and executable names. Existing installations can therefore be
upgraded without losing their catalogue or credentials. The public product name, repository, installer URLs,
...
truthful `provideros` identity and must succeed before the route can be
...
```sh
provideros key-pool opencode-go add-env OPENCODE_API_KEY
provideros key-pool opencode-go add-env OPENCODE_GO_API_KEY
provideros key-pool opencode-go policy round-robin
provideros key-pool opencode-go status
```
...
model_catalog_json = "/absolute/path/to/.codex/provideros/merged-models.json"
...
export CODEX_ROUTER_STATE_DIR="$CODEX_HOME/provideros"
```
...
```toml
model_catalog_json = "/mnt/c/Users/<WindowsUser>/.codex/provideros/merged-models.json"
```
...
so root-OpenAI configurations use the compatible `provideros` provider while
...
- `provideros` — orientation: how flattened `codex_app__` / `mcp__` tools
...
to `~/.codex/skills/` (each directory is marked `.provideros-managed`);
...
ownership to provideros:

```sh
./bin/provideros codex skills approve-external provideros
./bin/provideros codex skills revoke-external provideros
```
...
Approval never authorizes provideros to replace or remove the external
