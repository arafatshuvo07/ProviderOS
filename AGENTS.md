...
2. Use a stable checkout: `~/.local/share/provideros` on macOS/Linux, or
   `%LOCALAPPDATA%\provideros` on Windows. Do not install the service from a
   temporary clone.
...
1. **Both verbs go through `src/service.mjs`.** `bin/start`, Windows
   `provideros.ps1 start`, and their corresponding stop paths manage the same
...
2. **The foreground supervisor stays reachable, never by accident.**
   `bin/start --foreground` and `provideros.ps1 start --foreground` are the
