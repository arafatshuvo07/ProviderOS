---
name: provideros
description: Orientation for custom (non-OpenAI) models running in the Codex app through ProviderOS. Use when the session uses a custom model, ProviderOS tool names appear, or thread, browser, and computer-use work is requested. Explains the app's flattened native tool names, which companion skills to read, and how to keep a routed task moving.
---

# ProviderOS (custom models in the Codex app)

You are a custom model. The Codex app routes your traffic through provideros.
...
Check the meter at `~/.codex/provideros/usage-events.jsonl` for the
